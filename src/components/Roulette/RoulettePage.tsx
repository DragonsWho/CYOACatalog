// Public bump roulette page (/roulette): read-only public stats (GET /api/custom/bump/stats) —
// countdown to the draw, standings (games by ticket count), recent winners. Anonymous-friendly.
...
export default function RoulettePage() {
  ...
  // The countdown is computed in render, so without a tick it froze at page-load value.
              ...
              <Paper variant="outlined">
                <List disablePadding>
                  {stats.winners.map((w, i) => (
                    <Box component="li" key={`${w.id}-${w.drawn_at}`}>
                      {i > 0 && <Divider />}
                      <WinnerRow w={w} />
                    </Box>
                  ))}
