import { Box, Container, Typography, Link as MuiLink } from '@mui/material';
import { styled } from '@mui/material/styles';
import BoostyIcon from './BoostyIcon';
import PatreonIcon from './PatreonIcon';
import PatreonWordmark from './PatreonWordmark';

const StyledFooter = styled('footer')(({ theme }) => ({
  backgroundColor: theme.palette.primary.main,
  color: theme.palette.primary.contrastText,
  padding: theme.spacing(0.5, 0),
// marginTop:auto moved to the wrapper (FooterZone): the red stripe is no longer the last element —
// a dark row with the /all link sits above it; they must be pushed to the bottom together.
}));

const SupportLink = styled(MuiLink)(({ theme }) => ({
  display: 'inline-flex',
  alignItems: 'center',
  color: theme.palette.primary.contrastText,
  textDecoration: 'none',
  marginLeft: theme.spacing(0.5),
  marginRight: theme.spacing(0.5),
  '&:hover': {
    textDecoration: 'underline',
  },
}));

const IconWrapper = styled('span')({
  display: 'inline-flex',
  alignItems: 'center',
});

export default function Footer() {
  return (
    <Box sx={{ mt: 'auto' }}>
      {/*
        Link to the server-rendered catalog HTML index (/all, seo_index.go) for crawlers: one hop
        from every page to the list of all games, else bots see only 25 feed cards. Above the red
        stripe (that's the support-the-project sign, no service navigation there). Plain <a>, not
        RouterLink: /all is not a React route — Go serves it before the SPA catch-all, and router
        interception gives a white screen.
      */}
      <Box
        component="nav"
        sx={{
          display: 'flex',
          justifyContent: 'center',
          // No own background: the page background is lightly textured and a flat
          // background.default read as a pasted plaque.
          bgcolor: 'transparent',
          py: 1.25,
        }}
      >
        <MuiLink
          href="/all"
          sx={{
            color: 'text.secondary',
            fontSize: 13,
            textDecoration: 'none',
            '&:hover': { color: 'primary.main', textDecoration: 'underline' },
          }}
        >
          All games
        </MuiLink>
      </Box>
      <StyledFooter>
        <Container maxWidth="lg">
          <Box
            sx={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              flexWrap: 'wrap',
              '& > *': { my: 1 },
            }}
          >
            <Typography variant="body1" component="span" sx={{ mr: 0.5 }}>
              Made by Dragon&lsquo;s Whore! To keep this project from going to hell, please support it on
            </Typography>
            <SupportLink href="https://boosty.to/dragonswhore" target="_blank" rel="noopener noreferrer" aria-label="Boosty">
              <IconWrapper>
                <BoostyIcon sx={{ width: 100, height: 'auto', ml: -1 }} />
              </IconWrapper>
            </SupportLink>
            <Typography variant="body1" component="span" sx={{ mx: 0.5, ml: -0.7 }}>
              or
            </Typography>
            <SupportLink href="https://www.patreon.com/DragonsWhore" target="_blank" rel="noopener noreferrer" aria-label="Patreon">
              <IconWrapper sx={{ display: 'flex', alignItems: 'center' }}>
                <PatreonIcon sx={{ width: 23, height: 'auto' }} />
                <PatreonWordmark sx={{ width: 100, height: 'auto', ml: -0.5 }} />
              </IconWrapper>
            </SupportLink>
          </Box>
        </Container>
      </StyledFooter>
    </Box>
  );
}
