...
async function createNewTag(tagName: string): Promise<Tag> {
    try {
      ...
      // Hardcoded id of the "Custom" tag category.
      const customCategoryId = "phc2n4pqe7hxe36";
...
export default function CustomTagSelector({ value, onChange, availableTags, onTagsChange }: CustomTagSelectorProps) {
  ...
  async function handleCreateNewTag() {
    ...
    try {
      ...
      // Without catch a failed request left isCreatingTag=true forever.
  ...
  // Timer in a ref: debounce() was recreated each render, losing the previous timer id, so nothing
  // was ever cancelled and Levenshtein ran on every keypress.
  ...
  function debounce<A extends unknown[]>(callback: (...args: A) => void, wait: number) {
    return (...args: A) => {
      ...
      suggestTimerRef.current = window.setTimeout(() => callback(...args), wait);
    };
  }
