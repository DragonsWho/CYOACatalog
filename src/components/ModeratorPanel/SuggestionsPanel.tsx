// /moderator/suggestions — manual gate for user-suggested links from /add-next. Approve → state
// "queued" (the night script picks it up); decline → "dismissed" with an optional reason shown to
// the submitter. Before approving, moderators can classify inline (no schema change — fields
// already on game_pipeline_state):
// - Rating (type: SFW/Ecchi/NSFW/Extreme) → catalog rating tag + downstream NSFW gate
// (s06b_review_gate);
// - Fresh (data.original) → games.original_release (green badge / front-page pin for a genuine
// author release);
// - Note (data.mod_note) → PRIVATE note for the night pipeline agent (distinct from the decline
// reason).
// Edits save instantly via POST /suggestions/{id}/update and ride the record through the pipeline.
...
export default function SuggestionsPanel() {
  ...
  // Inline classification edit: trust the server echo and patch the local row from the response.
