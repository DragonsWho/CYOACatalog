...
export default function CyoaImageUploader({
  ...
  // Mirror for unmount cleanup: an effect on [images] would revoke URLs while they're displayed.
  const imagesRef = useRef<CyoaImage[]>([]);
  ...
  async function recreateImageArray(files: File[]): Promise<void> {
    ...
    // Revoke old blob URLs manually, or each rebuild (split, drop) leaks images until reload.
    const stale = images.map((img) => img.preview);
    setImages(newImages);
  ...
  function removeImage(index: number): void {
    const dropped = images[index]?.preview;
