...
import { LinkifiedText } from '../../utils/pbErrors';
...
export default function LoginDialog({
  ...
  // Reset on open: otherwise a dialog closed on "Sign up" reopened on registration.
  useEffect(() => { if (open) setMode(forcedMode ?? 'login'); }, [open, forcedMode]);
  ...
  const handleOAuth2Login = async (provider: AuthProviderInfo) => {
    ...
    try {
      ...
      // Ignore the error when the user closed the OAuth2 popup.
      ...
      return (
        <Fade in={true} timeout={300}>
          <Box component="form" onSubmit={handleLoginSubmit} sx={formSx}>
            ...
            {generalError && (
              <Alert severity="error" sx={{ mt: 2, borderRadius: 2 }}><LinkifiedText text={generalError} /></Alert>
            )}
          </Box>
        </Fade>
      );
