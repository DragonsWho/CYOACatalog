import { useState, useEffect, KeyboardEvent, ChangeEvent, useMemo } from 'react';
...
export default function AuthorSelector({ value, onChange, availableAuthors, onAuthorsChange }: AuthorSelectorProps) {
  ...
  const [createAuthorError, setCreateAuthorError] = useState<string | null>(null);
  ...
  const debouncedFindSuggestions = useMemo(
    () =>
  ...
  async function handleCreateOrSelectAuthor() {
    ...
    if (existingAuthor) {
      ...
      const created = await handleCreateNewAuthor();
      if (!created) return;
    }
  ...
  async function handleCreateNewAuthor() {
    ...
    try {
        ...
        setCreateAuthorError('Could not create author. Please try again.');
        return false;
    } finally {
