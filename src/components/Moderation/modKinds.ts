// Shared catalogue of moderator-request kinds. Mirrors the `kind` select on `mod_requests` and
// validModKinds in Go. The "Call a moderator" menu offers these with tooltip hints; each seeds the
// composer with a guided @moderator template. A stable shape helps the future LLM moderator parse
// requests.
...
export interface ModKindDef {
  ...
  // Seed text for the composer. @moderator stays on the first line; a blank line follows so the
  // cursor lands below it.
  seed: string;
}
