...
export const getTags = async () => {
  try {
    const firstPageResponse = await axios.get(
      `${API_URL}/api/tags?populate=tag_category&pagination[pageSize]=100&pagination[page]=1`,
    );
...
export const postComment = async (gameId, content, parentId = null) => {
  try {
    ...
    const response = await axios.post(`${API_URL}/api/comments/api::game.game:${gameId}`, commentData, {
      headers: {
...
export const deleteComment = async (gameId, commentId) => {
  try {
    ...
    console.log(
      `Deleting comment: ${API_URL}/api/comments/api::game.game:${gameId}/comment/${commentId}?authorId=${authorId}`,
    );
