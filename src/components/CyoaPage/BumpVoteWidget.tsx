// Bump vote ticket button in the action row (GameAdditionalInfo): vote count; popover with
// countdown to the draw, "my votes k/3", Vote (with a human reason when unavailable), undo within
// 24 h, link to public roulette stats. All rules live on the server (bump_roulette.go); here only
// status reads and intents via bumpRouletteApi.
...
export default function BumpVoteWidget({ gameId }: Props) {
  ...
  // Load on mount (public) and on auth change (my_tickets etc. depend on who asks).
  useEffect(() => {
    let cancelled = false;
