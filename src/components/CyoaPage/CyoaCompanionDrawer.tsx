...
// Calculator layer: above the site AppBar (1100) — the floating window can be dragged anywhere and
// must not go UNDER the header — but below quick chat (ChatDrawer backdrop 1199, drawer 1200).
// Previously button 1200 / window 1300 covered the open chat on static game pages. Change only
// together with ChatDrawer.
const CALC_BUTTON_Z = 1140;
const CALC_WINDOW_Z = 1150;
...
export default function CyoaCompanionDrawer({ gameId }: CyoaCompanionDrawerProps) {
  ...
  // Hide the floating Calc button while pinch-zoomed so it doesn't balloon over content.
  ...
  const [position, setPosition] = useState(() => {
    if (typeof window !== 'undefined') {
      return { 
        top: Math.max(50, window.innerHeight - 507),
        left: Math.max(50, window.innerWidth - 340)
      };
    }
  ...
  // Dice roller accepts a bare face count ("20") or NdM+K ("4d6+2", "d20-1").
  ...
  const handleResizeMove = (e: MouseEvent) => {
    ...
    // Position is top/left, so resizing grows right and down.
    setSize({
  ...
  // Opened by button only. Right-to-left swipe no longer opens the calculator: since chat v2
  // (2026-08-06) that gesture opens chat site-wide, and on static game pages one motion opened
  // both. Author's decision: the gesture belongs to chat; calculator via the floating "Calc"
  // button, closed via the header ×.
  const renderContent = () => (
    <>
      <Stack 
        direction="row" 
