// /moderator/tickets — moderator request queue. Tickets are created by Go (maybeCreateModRequest)
// when a comment contains @moderator. Persistent (unlike notifications): a ticket lives until
// closed. Read-only for clients; edits via Go /api/custom/mod-requests/update (re-checks
// isModerator).
...
// One-tap replies to drop into the public reply box, then edit.
...
export default function ModRequestsPanel() {
  ...
  // Local draft of internal notes keyed by ticket id (typing doesn't refetch).
  ...
  // Public reply (as a reply to the triggering comment), optionally resolving the ticket. Same Go
  // endpoint users use, so it fits the future LLM-moderator path. The requester gets a normal
  // "reply" notification (Go fan-out).
  ...
  return (
    <Container maxWidth="md">
      ...
      {loading ? (
        ...
        groups.map((g) => (
          <Box key={g.key}>
            {/* Group header only when a game has more than one open ticket. */}
            {g.tickets.length > 1 && (
