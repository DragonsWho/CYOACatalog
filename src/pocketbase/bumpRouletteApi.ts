// Thin client for the bump-roulette Go endpoints (bump_roulette.go). The vote lifecycle is
// server-authoritative: the frontend reads aggregates and posts intent; every rule (cooldown, caps,
// lockout) lives in Go.
...
async function jsonOrThrow(res: Response): Promise<any> {
  ...
  if (!res.ok) {
    ...
    // PocketBase ApiError puts our attached metadata (next_vote_at, …) under `data`.
