...
export default function ImageSplitter({
  ...
  useEffect(() => {
    setCrop(undefined);
    // Object URL instead of readAsDataURL: no base64 of multi-MB scans; also the FileReader had no
    // error handler and the dialog stayed empty forever on failure.
  ...
  async function onSplitImage() {
    // toBlob is async (two awaits): without a guard a double tap called returnImages twice,
    // duplicating split pairs.
    ...
    try {
      ...
      // toBlob returns null if the canvas didn't fit in memory; unchecked, the literal string
      // "null" went into a File and a broken png got hosted.
      if (!topBlob || !bottomBlob) return;
