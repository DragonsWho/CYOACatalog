// Bump roulette: community raffle deciding which game gets bumped to the top. A once-a-day vote =
// raffle ticket (max 3 per game per account); a minute cron draws one uniformly random ticket when
// now ≥ bump_settings.next_draw_at; winner gets bumped_at=now, tickets reset, 30d lockout.
// Server-authoritative like tag votes: bump_votes is backend-write-only, all rules enforced here,
// frontend reads aggregates only. Collections bump_votes/bump_draws/bump_settings auto-create on
// boot; games/users schema untouched. Canon: PB/schema_changes_bump_roulette.md
package main
...
// Non-fatal: on failure log and leave the feature dark rather than block boot.
...
func bumpEligibility(app core.App, game, s *core.Record, now time.Time) (ok bool, reason string, lockedUntil time.Time) {
	// Games are soft-deleted via hidden=true; there is no `deleted` field.
...
func performDraw(app core.App, s *core.Record, now time.Time) (string, error) {
	...
	err := app.RunInTransaction(func(tx core.App) error {
		...
		// Winner's tickets are consumed; the 30d lockout is implied by the new bump_draws row.
		_, err = tx.DB().
...
// NSFW is a tag, not a column: resolve "nsfw"/"extreme" ids case-insensitively, same as
// buildCatalogScriptTag in main.go.
...
func bumpStandings(app core.App, limit int) ([]bumpStanding, error) {
	// g.tags is raw relation JSON; substring match on the 15-char id is a reliable membership test.
	var rows []struct {
		ID      string `db:"id"`
		Slug    string `db:"slug"`
...
func registerBumpRoulette(app *pocketbase.PocketBase) {
	app.OnServe().BindFunc(func(e *core.ServeEvent) error {
		...
		g.POST("/bump/admin", func(c *core.RequestEvent) error {
			...
			bumpFields := []string{"enabled", "interval_hours", "min_age_days", "win_lockout_days", "max_tickets_per_game", "vote_cooldown_hours", "withdraw_window_hours", "next_draw_at"}
			bumpBefore := modSnapshot(s, bumpFields...)
		...
		g.POST("/bump/admin/draw", func(c *core.RequestEvent) error {
			...
			logModAction(app, c, modAction{
				...
				Target: winner,
				Game:   winner,
				After:  map[string]any{"winner": winner, "at": modWhen(now)},
				// A draw moves the game in the feed and consumes the ticket pool. No automatic undo.
				Reversible: false,
				Note:       "manual draw triggered from the moderator panel",
			})
