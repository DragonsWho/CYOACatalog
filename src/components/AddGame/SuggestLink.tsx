// Section ① of the add page: paste a link → the night pipeline does the rest. Uses POST
// /api/pipeline/submit (suggested state for regular users), GET /api/pipeline/my-submissions, GET
// /api/pipeline/suggestions/public.
...
// Cloudflare cuts request bodies around 100 MB, so big packs go in ~45 MB batches: the first
// creates the suggestion, the rest append via /api/pipeline/submit/{id}/pages. Only a single file
// over the per-request ceiling is impossible.
...
export default function SuggestLink() {
  ...
  // Default NSFW on purpose: a wrong "NSFW" hides a safe game behind a click; a wrong "SFW" leaks
  // porn onto the safe side.
  const [rating, setRating] = useState<'SFW' | 'Ecchi' | 'NSFW' | 'Extreme'>('NSFW');
  const [note, setNote] = useState('');
  const [files, setFiles] = useState<File[]>([]);
  // "I'm the author" — original release; pinned on the front page.
  ...
  const fetchMine = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Resolve pretty slugs in one batch so "published" links go to /game/<slug>; cosmetic,
        // failure keeps id links.
