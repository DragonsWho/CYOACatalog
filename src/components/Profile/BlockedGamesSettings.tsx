...
export default function BlockedGamesSettings({ blockedGameIds, onBlockedGamesUpdate }: BlockedGamesSettingsProps) {
  ...
  // Fetch ONLY id+title (payload discipline) via the anonymous cacheable client like the whole
  // catalog.
  useEffect(() => {
    ...
    gamesCollectionPublic
      ...
      .then((res) => {
        if (cancelled) return;
        // Keep blockedGameIds order (stable rather than jumping by created).
