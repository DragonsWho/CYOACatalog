...
// Personal-vote accent: colored border, edit mode only.
...
// Delete-ballot fill (accepted tag past the fade point, score <= -5): deep solid red + black
// diagonal stripes so the second "vote to fully remove" reads as more serious than the soft fade.
const DEL_FILL_RED = 'rgba(150,28,22,0.85)';

// Proposed-tag look: striped purple chip + dark green/red striped progress bar. Fill underlay
// hsl(106|4, 50%, 35%) at 75% opacity (tuned).
...
export default function TagChip({ tag, vote, isUpdating, editing = false, isGold = false, user, onClick }: TagChipProps) {
  ...
  // Bar geometry (pure, tagBar.ts). Proposed tags: small -5..+5 ballot. Accepted tags: two-phase
  // negative scale — "fade" (0→-5) then a fresh "delete" ballot (-5→-15) — so removal shows real
  // progress instead of a pegged bar.
  ...
  // Bars always full width, masked with clip-path: value changes re-clip in place (no sliding on
  // click).
  const clip =
    dir === 'pos'
  ...
  const styles: any = {
    ...
    // Resting (non-edit) chips are filter shortcuts: click opens header search with this tag. Edit
    // mode votes.
