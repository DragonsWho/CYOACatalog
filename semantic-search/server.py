...
PB_SUPERUSER_EMAIL/PB_SUPERUSER_PASSWORD (or EMAIL/PASSWORD, as in the old
system), optional SITE_API_URL (default https://cyoa.cafe/api), INDEX_DIR
(default ./index). With no index on disk the server starts empty and the first
sync builds the whole index (bootstrap for a fresh install).
...
INDEX = Path(os.environ.get("INDEX_DIR") or PILOT / "index")
...
if (INDEX / "catalog_vecs.npy").exists():
    VECS = np.load(INDEX / "catalog_vecs.npy")
    META = json.load(open(INDEX / "catalog_meta.json"))
else:  # fresh install: start empty, the first sync embeds the whole catalog
    VECS = np.zeros((0, int(os.environ.get("EMBED_DIM", "1536"))), dtype=np.float32)
    META = []
...
def sync_index() -> None:
    ...
    for g in games:
        if g.get("hidden"):
            continue  # hidden/"deleted" games are not indexed (semantic/similar)
    ...
    if (old_meta and len(to_embed) > max(50, 0.2 * len(games))
...
# A game's neighbours change only when the index is rebuilt (daily or less), yet EVERY game page
# requests this under "Similar games" — nearly every site visit. A day of Cloudflare + browser
# cache takes that load off the service entirely; the answer is public and user-independent.
SIMILAR_CACHE_SECONDS = int(os.environ.get("SIMILAR_CACHE_SECONDS", "86400"))


@app.get("/api/similar-games/{game_id}")
def similar_games(request: Request, response: Response, game_id: str):
    ...
    response.headers["Cache-Control"] = f"public, max-age={SIMILAR_CACHE_SECONDS}"
