...
import { AppBar, Toolbar, Typography, Box, Tooltip, Container, SvgIcon, useTheme } from '@mui/material'; // Убрали ToggleButton, ToggleButtonGroup
...
interface HeaderProps {
    ...
    filterMode: FilterMode; // <-- Получаем из App
    onFilterModeChange: (newMode: FilterMode) => void; // <-- Получаем из App
}
...
const Header: React.FC<HeaderProps> = ({
    ...
    onTagChange,
    onAuthorChange,
    filterMode, // Используем проп
    onFilterModeChange, // Используем проп
}) => {
    const { signedIn, user } = useContext(AuthContext);
    const [loginOpen, setLoginOpen] = useState(false);
    const theme = useTheme();

    // УДАЛЕНО: const handleFilterChange = ...
    // УДАЛЕНО: const getThumbStyles = ...
                ...
                <Container maxWidth="lg">
                    <Toolbar disableGutters>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', flexGrow: 1, justifyContent: 'flex-end', gap: 1.5 }}>
                            ...
                            {/* --- ИЗМЕНЕНИЯ ЗДЕСЬ --- */}
                            {/* Используем новый компонент FilterSwitch */}
                            <FilterSwitch
