"""
Embedding helper shared by the semantic-search server and offline tools.
...
Keys (GEMINI_API_KEY / GOOGLE_API_KEY; several = round-robin across quotas) come from the process
env, a .env next to this file (deployed server), or the file named by EMBED_KEYS_FILE.
EMBED_CACHE_DIR overrides the vector cache location (default: ./cache_vectors).
"""
...
CACHE_DIR = Path(os.environ.get("EMBED_CACHE_DIR") or PILOT_DIR / "cache_vectors")
...
def _read_keys() -> list[str]:
    files = [PILOT_DIR / ".env"]                     # deployed server
    if os.environ.get("EMBED_KEYS_FILE"):
        files.append(Path(os.environ["EMBED_KEYS_FILE"]))
    candidates = [(f, n) for f in files for n in ("GEMINI_API_KEY", "GOOGLE_API_KEY")]
    keys = []
